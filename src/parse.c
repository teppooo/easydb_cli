...
int check_header(struct dbheader_t *headerPtr, unsigned int filesize) {
  if (headerPtr->magic != HEADER_MAGIC || headerPtr->version != DB_VERSION ||
...
int read_employees(int fd, struct dbheader_t *headerIn,
...
int add_employee(struct dbheader_t *headerIn, struct employee_t **employeesOut,
                 char *addStr) {
  ...
  struct employee_t *employees =
      realloc(*employeesOut, sizeof(struct employee_t) * (headerIn->count + 1));
  if (employees == NULL) {
    perror("realloc");
    return STATUS_ERROR;
  }
  ...
  *employeesOut = employees;
  ...
  const int maxDigits =
      20; // should be enough for any unsigned int on any system
  char hourBuf[maxDigits];
  bzero(hourBuf, maxDigits);
  strncpy((char *)hourBuf, &(start[seek + 1]),
          maxDigits - 1); // always leave a null byte
  *hours = (unsigned int)strtol(hourBuf, NULL, 10);

  return STATUS_SUCCESS;
}

int list_employees(struct dbheader_t *headerIn,
                   struct employee_t *employeesIn) {
...
int output_file(int fd, struct dbheader_t *headerIn,
                struct employee_t *employees) {
  ...
  lseek(fd, 0, SEEK_SET);
  if (write(fd, headerIn, sizeof(struct dbheader_t)) <
      (ssize_t)sizeof(struct dbheader_t)) {
    perror("write header");
    return STATUS_ERROR;
  }
